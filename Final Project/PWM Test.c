...
unsigned int dutycycle = 0; //from 0 to 39,999
...
//----PID parameters-----
volatile unsigned int pid_p=0, pid_i=0, pid_d=0, pid_a=0;
double alpha = .5;
volatile double angle=0,scaled_angle=0,adc_scale=1.0/1023.0;
volatile int error=0;
volatile int prev_errors[4]={0,0,0,0};
...
void __ISR(_TIMER_2_VECTOR, ipl2) Timer2Handler(void)
{
    ...
    WriteSPI2(DAC_config_chan_A|(adcVal<<2)&0xfff);
...
//    tft_writeString(buffer);
    //sprintf(PT_send_buffer,"error: %d", error);
    //PT_SPAWN(pt, &pt_DMA_output, PT_DMA_PutSerialBuffer(&pt_DMA_output) );
    //prop control
    dutycycle=0;
    dutycycle+=pid_p*error;
    //diff control
    error_deriv = (prev_errors[3]-prev_errors[0])>>4;//approx derivative fct
   ...
   dutycycle = 28000;
    ...
    SPI_Mode16();
    mPORTBClearBits(BIT_4);
    WriteSPI2(DAC_config_chan_B|((dutycycle/10)&0xfff));
    while(SPI2STATbits.SPIBUSY){};//wait for that to transmit
    junk = ReadSPI2();
    mPORTBSetBits(BIT_4);
...
static struct pt pt_timer, pt_screen, pt_serial ;
            ...
            // no actual string
             ...
             switch(cmd[0]){
                 ...
                 case 'i': 
                    ...
                    pid_i=(int)value;
                    cursor_pos(6,1);
                    sprintf(PT_send_buffer,"Int Gain = %d", pid_i);
                    // by spawning a print thread
                    PT_SPAWN(pt, &pt_DMA_output, PT_DMA_PutSerialBuffer(&pt_DMA_output) );
                    clr_right ;
                    break;
                 ...
                 case 'a': 
                    ...
                    pid_a=(int) value;
                    cursor_pos(6,1);
                    sprintf(PT_send_buffer,"Angle = %d ", pid_a);
                    cursor_pos(7,1);
                    //pid_a=(int) ((value+90)/180.0)*1023+potbias;//pid_a is 0-1023
                    pid_a = (int) value + potbias;
                 ...
                 case 'l': 
                     // enter frequecy in HZ
                    cursor_pos(6,1);
                    sprintf(PT_send_buffer,"P, I, D, A = %d, %d, %d, %d", pid_p, pid_i, pid_d, pid_a);
                    // by spawning a print thread
                    PT_SPAWN(pt, &pt_DMA_output, PT_DMA_PutSerialBuffer(&pt_DMA_output) );
                    clr_right ;
                    //reset the motor arm to try the PID controller again
...
void main(void) {
 //SYSTEMConfigPerformance(PBCLK);
  ...
  //----------------------------------------------
    ...
    tft_fillScreen(ILI9340_BLACK);
    //potbias = ReadADC10(0); //the pendulum is hanging at zero, so this bias corresponds to -90
