...
volatile unsigned int FM_DDS_increment = FM_Fout*two32/Fs ;
// waveform amplitude
volatile _Accum max_amplitude=2000;
volatile _Accum FM_max_amplitude=1000,FM_out;
...
// used variables in main program
volatile int pushState=0;
volatile int nPossible=-1;
volatile int mode=0;
volatile int buttonPressed=0;
volatile int recording[100];
volatile int recIndex=0;
volatile int n=0;
volatile int isPlaying=0;
volatile int recState=0;
volatile int recStart=0;
volatile int note_to_play = 0;
volatile int badnote;
...
{
   PT_BEGIN(pt);
   static int keypad, i, pattern,recordButton;
   ...
   #define no_button (0x70)
     ...
     while(1) {
       ...
       for (i=0; i<4; i++) {
           ...
           keypad  = readPE(GPIOY);
           recordButton = readBits(GPIOZ,BIT_0);
           end_spi2_critical_section;
       ...
       switch(recState){
           ...
           case (0):
               if (recordButton){recState =0;}
               else if (0==recordButton){
                   recState=1;
               }
           break;
           case (1):
     ...
     // state machine for keypad
       ...
       // begin checking the state of the modes
       ...
       if (0==isPlaying){
        ...
        else if (recordButton == 0){
           if (recStart){
               recIndex =0;
               recStart =0;
           }
           mode =2;
       } // when the GPIOZ1 is pulled low, mode is 2
       else if (i<=8){mode =0;}
       
       else if (i>11){(mode =1);}
       else {mode =0;} // default case
       }
       
       
       if(mode == 3){   // play the recorded music when mode 3
        song_time=0;
       note_time=0;
       current_amplitude = 0 ;
         if(n<=recIndex){
           note_to_play = recording[n];
           switch(note_to_play){
           case 1: DDS_increment=262.0*two32/Fs ;    
                   FM_DDS_increment=DDS_increment *3;
                   badnote=0;
           break;
           case 2: DDS_increment=294.0*two32/Fs ;
                   FM_DDS_increment=DDS_increment *3;
                   badnote=0;

           break;
           case 3: DDS_increment=330.0*two32/Fs ;
                   FM_DDS_increment=DDS_increment *3;
                   badnote=0;

           break;
           case 4: DDS_increment=349.0*two32/Fs ;
                   FM_DDS_increment=DDS_increment *3;
                   badnote=0;

           break;
           case 5: DDS_increment=392.0*two32/Fs ;
                   FM_DDS_increment=DDS_increment *3;
                   badnote=0;

           break;
           case 6: DDS_increment=440.0*two32/Fs ;
                   FM_DDS_increment=DDS_increment *3;
                   badnote=0;

           break;
           case 7: DDS_increment=494.0*two32/Fs ;
                   FM_DDS_increment=DDS_increment *3;
                   badnote=0;

           break;
           case 8: DDS_increment=523.0*two32/Fs ;
                   FM_DDS_increment=DDS_increment *3;
                   badnote=0;

           break;
           default: DDS_increment = 0;
                   FM_DDS_increment = 0;
                   FM_current_amplitude = 0;
                   badnote=1;
               break; // Not sure 
         ...
         PT_YIELD_TIME_msec(300); // yield half second between notes
       ...
       switch(i){
           ...
           default: DDS_increment = 0;
              ...
              switch(i){
           case 13: DDS_increment=262.0*two32/Fs ;          
           break;
           case 14: DDS_increment=294.0*two32/Fs ;
           break;
           case 15: DDS_increment=330.0*two32/Fs ;
           break;
           case 16: DDS_increment=349.0*two32/Fs ;
           break;
           case 17: DDS_increment=392.0*two32/Fs ;
           break;
           case 18: DDS_increment=440.0*two32/Fs ;
           break;
           case 19: DDS_increment=494.0*two32/Fs ;
           break;
           case 20: DDS_increment=523.0*two32/Fs ;
           break;
           default: DDS_increment = 0;
               break; // Not sure 
       ...
       // draw key number
        sprintf(buffer," mode %x * ,i %x,r %x", buttonPressed,i,recIndex);
/*
...
//SYSTEMConfigPerformance(PBCLK);
 ...
 // timer interrupt //////////////////////////
   ...
   OpenTimer2(T2_ON | T2_SOURCE_INT | T2_PS_1_1, 910);
  ...
  // bow parameters range check
	if (FM_attack_time < 1) FM_attack_time = 1;
	if (FM_decay_time < 1) FM_decay_time = 1;
	if (FM_sustain_time < 1) FM_sustain_time = 1;
